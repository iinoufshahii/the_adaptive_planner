...
''';
  ...
  Future<void> _saveSubtasks() async {
    final selectedSubtasks = _selectedSubtasks
        .map((index) => _suggestedSubtasks[index])
        .toList();
    
    if (selectedSubtasks.isEmpty || widget.task.id == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select at least one subtask')),
      );
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Subtasks saved successfully!'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to save subtasks: $e'),
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildInfoRow('Category', task.category.name.toUpperCase(),
                        theme.colorScheme.primary),
                    const SizedBox(height: 8),
                    _buildInfoRow('Energy Level',
                        task.requiredEnergy.name.toUpperCase(), mintGreen),
                    const SizedBox(height: 8),
                    _buildInfoRow(
                        'Due', _formatDeadline(task.deadline), Colors.orange),
                  ],
                ),
              ),
            ),
                        ...
                        onPressed: () async {
                          ...
                          if (mounted) {
                            ScaffoldMessenger.of(buildContext).showSnackBar(
                              const SnackBar(content: Text('Subtasks cleared')),
                            );
                          }
                        },
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        onPressed:
                            _isGeneratingSubtasks ? null : _generateSubtasks,
                    ...
                    if (_suggestedSubtasks.isNotEmpty) ...[
                      ...
                      const SizedBox(height: 12),
                      ...List.generate(_suggestedSubtasks.length, (index) {
